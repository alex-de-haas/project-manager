    ...
    `
      WITH tracked_items AS (
        SELECT
          ...
          wi.title,
          COALESCE(wi.created_by_user_id, wi.assigned_user_id) AS added_by_user_id,
          wi.created_at
        FROM work_items wi
        WHERE wi.assigned_user_id IS NOT NULL
          AND wi.type IN ('task', 'bug')
          AND (
            wi.sync_state = 'not_synced'
            OR wi.parent_work_item_id IS NULL
            OR EXISTS (
              SELECT 1
              FROM time_entries te
              WHERE te.work_item_id = wi.id
                AND te.user_id = wi.assigned_user_id
                AND te.hours > 0
            )
          )
      )
      ...
      SELECT
        ...
        ROW_NUMBER() OVER (
          ...
          ORDER BY lower(title) ASC, work_item_id ASC
        ) - 1,
        added_by_user_id,
        created_at,
        CURRENT_TIMESTAMP
    ...
    `
      WITH tracked_items AS (
        SELECT
          ...
          wi.id AS work_item_id,
          wi.title,
          te.user_id AS added_by_user_id,
          MIN(te.created_at) AS created_at
        FROM time_entries te
        INNER JOIN work_items wi ON wi.id = te.work_item_id
        WHERE wi.type IN ('task', 'bug')
          AND te.hours > 0
        GROUP BY wi.project_id, te.user_id, wi.id, wi.title
      )
      INSERT OR IGNORE INTO time_tracking_items (
        project_id,
        user_id,
